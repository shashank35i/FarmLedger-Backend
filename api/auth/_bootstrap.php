...
function send_email_otp(string $to, string $otp, string $purpose): bool {
    ...
    $subject = $purpose === 'RESET_PASSWORD'
        ? "FarmLedger password reset code"
        : "FarmLedger email verification code";
...
function otp_email_template(string $otp, string $purposeText, int $ttlMinutes): string {
    $brand = 'FarmLedger';
    $supportEmail = defined('SUPPORT_EMAIL') ? SUPPORT_EMAIL : (defined('SMTP_FROM_EMAIL') ? SMTP_FROM_EMAIL : 'support@farmledger.local');
    $year = date('Y');
